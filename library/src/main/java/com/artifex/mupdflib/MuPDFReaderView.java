...
public class MuPDFReaderView extends ReaderView {
	...
	protected void onChildSetup(int i, View v) {
		...
		if (SearchTaskResult.get() != null && SearchTaskResult.get().searchBoxes != null&& SearchTaskResult.get().searchBoxes.size()>0) {
			try {
				((MuPDFView) v).setSearchBoxes(SearchTaskResult.get().searchBoxes.get(i));
			}catch (Exception e){
				Log.i("SearchTaskResult.get",e.getMessage());
			}
			//((MuPDFView) v).setSearchBoxesPrim(SearchTaskResult.get().searchBoxesPrims.get(i));
		}
		else {
	...
	protected void onMoveToChild(int i) {
		if (SearchTaskResult.get() != null && SearchTaskResult.get().searchBoxes != null&&
				SearchTaskResult.get().searchBoxes.size()>0) {
