...
namespace wtd
{
	...
	void WTDE26Parser::Decode(WTD_ITEM1_TYPE type, const char * data, size_t length, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		Status s;
		bool ret = true;
	...
	void WTDE26Parser::DecodeEoas2(const unsigned char * data, size_t length, std::vector< TrainFault > & fault)
	{
		...
		for(int i = 0;i<12;i++)
		{
			...
			{
			case 0:
				InsertWtdItem2(_f,i,data,length,m_E26uiNo01Center1,sizeof(m_E26uiNo01Center1)/sizeof(m_E26uiNo01Center1[0]));
				break;
			case 1:
				InsertWtdItem2(_f,i,data,length,m_E26uiNo01Center2,sizeof(m_E26uiNo01Center2)/sizeof(m_E26uiNo01Center2[0]));
				break;
			case 2:
				InsertWtdItem2(_f,i,data,length,m_E26uiNo08Center1,sizeof(m_E26uiNo08Center1)/sizeof(m_E26uiNo08Center1[0]));
				break;
			case 3:
				InsertWtdItem2(_f,i,data,length,m_E26uiNo08Center2,sizeof(m_E26uiNo08Center2)/sizeof(m_E26uiNo08Center2[0]));
				break;
			case 4:
				InsertWtdItem2(_f,i,data,length,m_E26uiNo01Terminal,sizeof(m_E26uiNo01Terminal)/sizeof(m_E26uiNo01Terminal[0]));
				break;
			case 5:
				InsertWtdItem2(_f,i,data,length,m_E26uiNo02Terminal,sizeof(m_E26uiNo02Terminal)/sizeof(m_E26uiNo02Terminal[0]));
				break;
			case 6:
				InsertWtdItem2(_f,i,data,length,m_E26uiNo03Terminal,sizeof(m_E26uiNo03Terminal)/sizeof(m_E26uiNo03Terminal[0]));
				break;
			case 7:
				InsertWtdItem2(_f,i,data,length,m_E26uiNo04Terminal,sizeof(m_E26uiNo04Terminal)/sizeof(m_E26uiNo04Terminal[0]));
				break;
			case 8:
				InsertWtdItem2(_f,i,data,length,m_E26uiNo05Terminal,sizeof(m_E26uiNo05Terminal)/sizeof(m_E26uiNo05Terminal[0]));
				break;
			case 9:
				InsertWtdItem2(_f,i,data,length,m_E26uiNo06Terminal,sizeof(m_E26uiNo06Terminal)/sizeof(m_E26uiNo06Terminal[0]));
				break;
			case 10:
				InsertWtdItem2(_f,i,data,length,m_E26uiNo07Terminal,sizeof(m_E26uiNo07Terminal)/sizeof(m_E26uiNo07Terminal[0]));
				break;
			case 11:
				InsertWtdItem2(_f,i,data,length,m_E26uiNo08Terminal,sizeof(m_E26uiNo08Terminal)/sizeof(m_E26uiNo08Terminal[0]));
				break;
			default:break;
			}
	...
	//解析UTC时间
	void WTDE26Parser::DecodeUtcTime(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		std::string utcTime;
		if (1 == ((*data) & 0x01))
		{
	...
	//提取门的状态
	void WTDE26Parser::DecodeDoor(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
	...
	//提取占用端记录
	void WTDE26Parser::DecodeOccupy(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		switch( (int)(*data) )
		{
		case 0x01:
	...
	//提取动车组速度
	void WTDE26Parser::DecodeSpeed(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
	...
	//提取联挂状态
	void WTDE26Parser::DecodeGroup(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		switch( (int)(*data) )
		{
		case 0x00:
	...
	//提取受电弓上升检测
	void WTDE26Parser::DecodePanto(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{

		std::string state;
		int _0bit = (int)(*data) & 0x01;
		int _1bit = (int)(*data) & 0x02;
		if(_0bit > 0)
		{
			state = "升;";
		}
		...
		s = PublicFun::InitProcData(WTD_ITEM1_PANTO,0,state);
	}

	//提取主断路器状态
	void WTDE26Parser::DecodeVCB(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{

		std::string state;
		int _0bit = (int)(*data) & 0x01;
		int _2bit = (int)(*data) & 0x04;
		...
		s = PublicFun::InitProcData(WTD_ITEM1_VCB,0,state);
	}

	//提取牵引手柄级位
	void WTDE26Parser::DecodeTowLevel(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		// 0xFF表示无效
		if( (int)(*data) == 0xFF )
		{
	...
	//提取制动手柄级位
	void WTDE26Parser::DecodeBrakeLevel(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		// 0xFF表示无效
		if( (int)(*data) == 0xFF )
		{
	...
	//提取停放制动状态
	void WTDE26Parser::DecodePB(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		std::ostringstream os;
		std::bitset<8> bits = std::bitset<8>( *data );
	...
	//提取分项区间M613
	void WTDE26Parser::DecodeSplitPhase(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		std::ostringstream os;
		std::bitset<8> bits = std::bitset<8>( *data );
	...
	//提取BIN
	void WTDE26Parser::DecodeBIN(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		const char * _p = data;
		std::ostringstream os;
	...
	//提取紧急制动拉动开关
	void WTDE26Parser::DecodeEBSwitch(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		std::ostringstream os;
		std::bitset<8> bits = std::bitset<8>( *data );
		...
		std::string strRet = os.str();
		PublicFun::eraseLastSymbol(strRet);
		s = PublicFun::InitProcData(WTD_ITEM1_EB_SWITCH,uiRet,os.str());
	}

	//提取制动指令
	void WTDE26Parser::DecodeBrakeInstruct(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		std::ostringstream os;
		std::bitset<8> bits = std::bitset<8>( *data );
	...
	//提取方向指令
	void WTDE26Parser::DecodeDirectInstruct(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		std::string strCmd = "";
		std::bitset<8> bits = std::bitset<8>( *data );
	...
	//提取恒速状态
	void WTDE26Parser::DecodeConstSpdState(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		const char * _p = data;
		// 恒速投入、切除状态
		if(0 == int(*_p))
	...
	//解析蓄电池电压
	void WTDE26Parser::DecodeStorageVol(const char * data, size_t length, Status & s,std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		//定义7个字节的数据
		std::string vols;
		const char * _p = data;
	...
	//解析缸压
	void WTDE26Parser::DecodeSylinder(const char * data, size_t length, Status & s,std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		//传9个字节的数据
		std::string press;
		const char * _p = data;
	...
	//解析牵引电流
	void WTDE26Parser::DecodePullElec(const char * data, size_t length, Status & s, std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		//传7个字节的数据，第一位判断有效位
		const char *_p = data;
		std::string current;
	...
	//解析再生制动力
	void WTDE26Parser::DecodeReBrake(const char * data, size_t length, Status & s,std::map<WTD_ITEM1_TYPE,Status> & status)
	{
		//传人6个字节的数据
	...
	//在二包中插入数据
	void WTDE26Parser::InsertWtdItem2(std::vector< TrainFault > & fault, unsigned int uiTrainDev, const unsigned char* pszData, const int iLen, const unsigned int uiStatMap[][3],const int iRowSize)
	{
		TrainFault _f;
